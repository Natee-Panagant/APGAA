function [AC, AcControl, AcTrim]=Geo_DLR_Validation_FullAC(state)
...
AC(1).SubSurf(1).SpanData=[0 1 40];% span percentage interval, nspan
AC(1).SubSurf(1).ChordIntv=[0 1];%chord percentage interval
AC(1).SubSurf(1).ChordMesh=[10];%nchord1, nchord2
...
AC(2).Config.Dihedral_ref = 1;% = 1 -> rotate around x-axis, = 2 -> rotate around Proot
AC(2).Config.Symmetry='no';% symmetrical wing

% Aero Analysis (Gen Panel)   
AC(2).SubSurf(1).Name='Vertical Tail';% aileron surface
AC(2).SubSurf(1).SpanData=[0 1 20];% span percentage interval, nspan
AC(2).SubSurf(1).ChordIntv=[0 1];%chord percentage interval
AC(2).SubSurf(1).ChordMesh=[10];%nchord1, nchord2
...
AC(3).SubSurf(1).SpanData=[0 1 20];% span percentage interval, nspan
AC(3).SubSurf(1).ChordIntv=[0 1];%chord percentage interval
AC(3).SubSurf(1).ChordMesh=[10];%nchord1, nchord2
...
AC(4).SubSurf(1).ChordMesh=[40];%nchord1, nchord2
